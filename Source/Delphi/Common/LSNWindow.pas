...
//    FontSize, 0, 0, 0, PChar(Text), Effect, RunSpeed);
  ...
  //添加文件区域  有问题的接口
  AddFileArea(sc.ID, 1, ID, X, Y, Width, Heigth);

  //添加字符串到图文区域中    有问题的接口
  AddFileString(sc.ID, 1, ID, 1, PChar(Text),
    PChar(FontName), FontSize, Color, 0, 0, 0, Alignment, Width,
    Heigth, Effect, 255, RunSpeed, StayTime, 1, 1);
